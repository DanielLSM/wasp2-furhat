...
# insocket.connect('http://' + config["Furhat_IP"] + ':8080/video')
# insocket.setsockopt(zmq.RCVHWM, 1)
# insocket.setsockopt(
#     zmq.CONFLATE,
#     1)  # Only read the last message to avoid lagging behind the stream.

##################hacky stuff
video_socket = 'http://' + config["Furhat_IP"] + ':8080/video'
capturer = cv2.VideoCapture(video_socket)

# Output results using a PUB socket
context2 = zmq.Context()
outsocket = context2.socket(zmq.PUB)
outsocket.bind("tcp://" + config["Dev_IP"] + ":" +
               config["detection_exposure_port"])
...
detection_threshold = config[
    "detection_confidence_threshold"]  # Detection threshold takes a double between 0.0 and 1.0
x = True
while x:
    # for a in range(2):
